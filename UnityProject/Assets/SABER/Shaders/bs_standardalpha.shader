...
Shader "BeatSaber/StandardAlpha(cookie)"
{
	Properties
    {
		// Glow
		...
		_cutoutcookie ("Cutout", Range (0, 1)) = 0.5
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            float4 frag (v2f i) : SV_Target
            {
				...
				// albedo Texture
				fixed4 _col = tex2D(_MainTex, TRANSFORM_TEX(i.uv, _MainTex));
				if (_col.a < _cutoutcookie) discard;
